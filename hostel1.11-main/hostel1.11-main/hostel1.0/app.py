...
def edit_canteen_menu(menu_id):
    ...
    if request.method == 'POST':
        try:
            ...
            menu_item.day = request.form.get('day')  # Ensure this is not None
            
            # Convert times 
            start_time_str = request.form.get('start_time')
            end_time_str = request.form.get('end_time')
            
            menu_item.start_time = datetime.strptime(start_time_str, '%H:%M').time()
            menu_item.end_time = datetime.strptime(end_time_str, '%H:%M').time()
            ...
            # Return to the edit page with the existing menu item
...
def admin_canteen_feedback():
    ...
    # Prepare feedback data with safe date handling
    processed_feedback = []
    for item in feedback:
        feedback_item, meal_type, menu_date, student_name = item
        
        # Create a dictionary with safe date formatting
        processed_item = {
            'feedback': feedback_item,
            'meal_type': meal_type,
            'date': menu_date.strftime('%Y-%m-%d') if menu_date else 'N/A',
            'student_name': student_name
        }
        processed_feedback.append(processed_item)
    
    return render_template('admin/canteen_feedback.html', feedback=processed_feedback)
